...
| Temporary taskpane verification surface | Added under Settings -> Search -> "OpenClaw Phase 0 diagnostics". It stores no config or token and sends no gateway traffic until a user presses `Test HTTP response` or `Test WS handshake`. After the first sideload run, the WS client id was changed from a SheetClaw-specific string to the documented `cli` client id because the gateway rejected unknown ids. |
| Reachability from taskpane webview | First sideload run: HTTP `POST /v1/responses` failed in browser with `Failed to fetch` before a readable response (likely CORS, endpoint exposure, mixed-content, auth, or reachability). WS did reach the gateway and received a protocol response, but `connect` failed validation because the diagnostic used an invalid `client.id`; retest pending with `client.id = "cli"`. |
